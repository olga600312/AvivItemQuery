package com.aviv_pos.olgats.avivitemquery.dao;
...
import com.aviv_pos.olgats.avivitemquery.beans.Promo;
import com.aviv_pos.olgats.avivitemquery.beans.Rule;
import com.aviv_pos.olgats.avivitemquery.beans.ZReport;
import com.aviv_pos.olgats.avivitemquery.beans.Item;
import com.aviv_pos.olgats.avivitemquery.beans.KeyValue;
import com.aviv_pos.olgats.avivitemquery.beans.KeyValueSet;
import com.aviv_pos.olgats.avivitemquery.beans.Store;
...
public class DatabaseHandler {
    ...
    public abstract static class Master<T extends DAOHelper.MASTER> {
        ...
        public List<KeyValue> retrive() {
            SQLiteDatabase db = helper.getReadableDatabase();
            SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
            queryBuilder.setTables(t.TABLE_NAME);
            ArrayList<KeyValue> arr = new ArrayList<>();
            try (Cursor cursor = queryBuilder.query(db, new String[]{t.ID, t.NAME}, null, null, null, null, null);) {
                if (cursor.moveToFirst()) {
                    final int valueColumn = cursor.getColumnIndex(t.NAME);
    ...
    public static class Items {
        ...
        public List<Item> retrieve(boolean extra) {
            ...
            try (Cursor cursor = database.query(DAOHelper.ITEMS.TABLE_NAME, null, null, null, null, null, DAOHelper.ITEMS.NAME);) {
                items.addAll(retrieveItems(cursor, extra));
            }

            return items;
        }

        private ArrayList<Item> retrieveItems(Cursor cursor, boolean extra) {
            ...
            if (cursor.moveToFirst()) {
                ...
                final int supplierColumn = cursor.getColumnIndex(DAOHelper.ITEMS.SUPPLIER);
                do {
                    Item item = new Item();
                    ...
                    item.setSupplier(cursor.getInt(supplierColumn));
        ...
        public Item retrieveItem(String code, boolean extra) {
            ...
            try (Cursor cursor = queryBuilder.query(database, null, null, null, null, null, null);) {
                items = retrieveItems(cursor, extra);
            }
        ...
        public Hashtable<String, Object> getExtra(String code) {
            ...
            SQLiteDatabase db = helper.getReadableDatabase();
            try (Cursor cursor = queryBuilder.query(db, new String[]{DAOHelper.EXTRA.NAME, DAOHelper.EXTRA.VALUE}, null, null, null, null, null);) {
                if (cursor.moveToFirst()) {
        ...
        public boolean init(Item arr[]) {
            ...
            for (Item item : arr) {
                Object oSupplier = item.getExtra().get("supplier");
                if (oSupplier != null && !oSupplier.toString().isEmpty()) {
                    item.setSupplier(Integer.parseInt(oSupplier.toString()));
                }
                create(item);
            }
            return true;
        ...
        public Item create(Item item) {
            ...
            values.put(DAOHelper.ITEMS.WEIGHT, item.isWeightable() ? 1 : 0);
            values.put(DAOHelper.ITEMS.SUPPLIER, item.getSupplier());
            if (database.insert(DAOHelper.ITEMS.TABLE_NAME, null, values) >= 0) {
                insertExtra(item, database, values, code);
            }
            return item;
        }
    ...
    public static class ItemPromos {
        ...
        public List<Promo> retrieve(String barcode) {
            ...
            SQLiteDatabase database = helper.getReadableDatabase();
            try (Cursor cursor = database.query(DAOHelper.ITEM_PROMO.TABLE_NAME, null, DAOHelper.ITEM_PROMO.BARCODE + "=?", new String[]{barcode}, null, null, DAOHelper.ITEM_PROMO.ID);) {
                if (cursor.moveToFirst()) {
    ...
    public static class Rules {
        ...
        public boolean getValue(String key) {
            boolean result = true;
            SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
            queryBuilder.setTables(DAOHelper.RULE.TABLE_NAME);
            queryBuilder.appendWhere(DAOHelper.RULE.ID + " = '" + key + "'");
            SQLiteDatabase db = helper.getReadableDatabase();
            try (Cursor cursor = queryBuilder.query(db, new String[]{DAOHelper.RULE.VALUE}, null, null, null, null, null);) {
                if (cursor.moveToFirst()) {
                    final int valueColumn = cursor.getColumnIndex(DAOHelper.RULE.VALUE);
                    do {
                        try {
                            result = cursor.getInt(valueColumn) > 0;
                        } catch (Exception ex) {
                            result = true;
                        }
                    }
        ...
        public boolean setValue(String key, boolean value) {
            SQLiteDatabase database = helper.getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put(DAOHelper.RULE.ID, key);
            values.put(DAOHelper.RULE.VALUE, value ? 1 : 0);
            return database.insert(DAOHelper.SETTINGS.TABLE_NAME, null, values) > 0;
        }


        public boolean replaceValue(String key, boolean value) {
         ...
         * @return
         */
        public boolean init(Rule arr[]) {
            SQLiteDatabase database = helper.getWritableDatabase();
            database.delete(DAOHelper.RULE.TABLE_NAME, null, null);
            for (Rule item : arr) {
        ...
        public Rule create(Rule rule) {
            SQLiteDatabase database = helper.getWritableDatabase();
            return create(rule, helper.getWritableDatabase());
        }

        public Rule create(Rule rule, SQLiteDatabase database) {
            ContentValues values = new ContentValues();
            values.put(DAOHelper.RULE.ID, rule.getKey());
            values.put(DAOHelper.RULE.VALUE, rule.getValue());
            return database.insert(DAOHelper.RULE.TABLE_NAME, null, values) >= 0 ? rule : null;
        }
